...
class PokemonDataModule(LightningDataModule):
    ...
    def get_val_dataset(self, without_synth=False, overwrite_transforms=None):

        df = self.val_df if not without_synth else self.val_df.query("ds_type == 'og'")

        transform = overwrite_transforms or self.val_and_pred_transforms

        return RevisedDataset(df, "val", "crop", transform, imgsz=self.imgsz)

    def get_test_dataset(self, overwrite_transforms=None):

        transform = overwrite_transforms or self.val_and_pred_transforms

        return RevisedDataset(self.test_df, "test", "crop", transform, imgsz=self.imgsz)
    ...
    def create_split(self):
        ...
        if current_val_proportion < self.fill_val_to:
            required_additional_val = int(
                len(df_without_test) * self.fill_val_to - len(val_index)
            )
    ...
    def val_dataloader(self, without_synth=False, overwrite_transforms=None):
        return DataLoader(
            self.get_val_dataset(
                without_synth=without_synth, overwrite_transforms=overwrite_transforms
            ),
            batch_size=self.batch_size,
            num_workers=self.num_workers,
            pin_memory=self.pin_memory,
    ...
    def test_dataloader(self, overwrite_transforms=None):
        return DataLoader(
            self.get_test_dataset(overwrite_transforms=overwrite_transforms),
            batch_size=self.batch_size,
            num_workers=self.num_workers,
            pin_memory=self.pin_memory,
        )
